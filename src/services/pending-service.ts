...
export class PendingManager {
  ...
  private setPendings = ({ fileName, roomId, pendings }: SetPendingProps): void => {
  ...
  isPendingExist = ({ version: v, socketId: s, ...docInfo }: IsPendingExistProps): boolean => {
    const pendings = this.getPendings(docInfo);

    return pendings.some(({ version, socketId }) =>
      v === version && s === socketId);
  }
